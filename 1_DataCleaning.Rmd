...
In previous exercises, we analyzed data as continuous-space, continuous-time stochastic processes.  Here, we will investigate the animal trajectory to identify changes in animal behavior.  Data, however, will be analyzed in discrete time.  

As a first step, we must first clean the data received before taking any additional advanced steps. This includes filtering the dataset for completeness, identifying duplicate records, removing invalid start or stop dates, identifying positions of poor data quality, and ordering the dataset sequentially so that we can import the data into `moveHMM` for analyses.  Nearly all manufacturers report some type of positional quality, often reported as the type of position (e.g., 1D, 2D, or 3D) or Dilution of Position (DOP - horizontal or vertical).  Large DOPs indicate poor positional quality that can be filtered from the dataset.
